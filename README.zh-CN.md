...
- 📸 **照片自动备份** —— 只要手机和电脑在同一个局域网，新照片自动抵达电脑。
- 🔀 **手机 ↔ 电脑直连传输** —— 任意文件、任意大小，走局域网原生 TCP，跑满带宽。
- 🖼 **时间线与地图视图** —— 真正的相册体验，而不是冷冰冰的目录树。
- 🤖 **本地 AI** —— 端侧去重与人脸分组（*即将推出*）。所谓"智能"，绝不上传任何东西。
- 🔐 **端到端加密** —— 现已具备 ed25519 设备身份；X25519 + ChaCha20-Poly1305 传输加密将在 v0.2 落地。
